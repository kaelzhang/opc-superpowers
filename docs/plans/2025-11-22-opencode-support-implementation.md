...
> **For agentic workers:** REQUIRED SUB-SKILL: Use opc-superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add full opc-superpowers support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
**Tech Stack:** Node.js, JavaScript, OpenCode Plugin API, Git
...
- Reference: `.codex/opc-superpowers-codex` (lines 40-74)
...
- Reference: `.codex/opc-superpowers-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'opc-superpowers' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/opc-superpowers-codex` (lines 212-280)
 ...
 * (personal skills override opc-superpowers skills).
 *
 * @param {string} skillName - Name like "opc-superpowers:brainstorming" or "my-skill"
 * @param {string} opcSuperpowersDir - Path to opc-superpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, opcSuperpowersDir, personalDir) {
    // Strip opc-superpowers: prefix if present
    const forceOpcSuperpowers = skillName.startsWith('opc-superpowers:');
    const actualSkillName = forceOpcSuperpowers ? skillName.replace(/^opc-superpowers:/, '') : skillName;

    // Try personal skills first (unless explicitly opc-superpowers:)
    if (!forceOpcSuperpowers && personalDir) {
    ...
    // Try opc-superpowers skills
    if (opcSuperpowersDir) {
        const opcSuperpowersPath = path.join(opcSuperpowersDir, actualSkillName);
        const opcSuperpowersSkillFile = path.join(opcSuperpowersPath, 'SKILL.md');
        if (fs.existsSync(opcSuperpowersSkillFile)) {
            return {
                skillFile: opcSuperpowersSkillFile,
                sourceType: 'opc-superpowers',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/opc-superpowers-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/opc-superpowers-codex` (add import at top)
...
Run: `node -c .codex/opc-superpowers-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/opc-superpowers-codex
...
**Files:**
- Modify: `.codex/opc-superpowers-codex` (lines 40-74)
...
Run: `.codex/opc-superpowers-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/opc-superpowers-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/opc-superpowers-codex` (lines 16-38, approximately)
...
Run: `.codex/opc-superpowers-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/opc-superpowers.js`
...
/**
 * opc-superpowers plugin for OpenCode.ai
 *
...
const opcSuperpowersSkillsDir = path.join(homeDir, '.config/opencode/opc-superpowers/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const opcSuperpowersPlugin = async ({ project, client, $, directory }) => {
  return {
...
Run: `ls -l .opencode/plugin/opc-superpowers.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/opc-superpowers.js
...
**Files:**
- Modify: `.opencode/plugin/opc-superpowers.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "opc-superpowers:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > opc-superpowers)
          const resolved = skillsCore.resolveSkillPath(
            skill_name,
            opcSuperpowersSkillsDir,
            personalSkillsDir
          );
...
Run: `node -c .opencode/plugin/opc-superpowers.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the opc-superpowers and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const opcSuperpowersSkills = skillsCore.findSkillsInDir(
            opcSuperpowersSkillsDir,
            'opc-superpowers',
            3
          );
          ...
          const allSkills = [...personalSkills, ...opcSuperpowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install opc-superpowers skills to ~/.config/opencode/opc-superpowers/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'opc-superpowers:';
...
```javascript
    'session.started': async () => {
      // Read using-opc-superpowers skill content
      const usingopcSuperpowersPath = skillsCore.resolveSkillPath(
        'using-opc-superpowers',
        opcSuperpowersSkillsDir,
        personalSkillsDir
      );

      let usingOpcSuperpowersContent = '';
      if (usingopcSuperpowersPath) {
        const fullContent = fs.readFileSync(usingopcSuperpowersPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingOpcSuperpowersContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- opc-superpowers skills: \`opc-superpowers:skill-name\` (from ~/.config/opencode/opc-superpowers/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/opc-superpowers')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/opc-superpowers && git pull` to update opc-superpowers.' :
        '';

      // Return context to inject into session
      return {
        context: `<EXTREMELY_IMPORTANT>
You have opc-superpowers.

**Below is the full content of your 'opc-superpowers:using-opc-superpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingOpcSuperpowersContent}
...
### 1. Install opc-superpowers Skills

```bash
# Clone opc-superpowers skills to OpenCode config directory
mkdir -p ~/.config/opencode/opc-superpowers
git clone https://github.com/kaelzhang/opc-superpowers.git ~/.config/opencode/opc-superpowers
```

### 2. Install the Plugin

The plugin is included in the opc-superpowers repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/opc-superpowers/.opencode/plugin/opc-superpowers.js`
...
ln -s ~/.config/opencode/opc-superpowers/.opencode/plugin/opc-superpowers.js .opencode/plugin/opc-superpowers.js
```
...
```
use use_skill tool with skill_name: "opc-superpowers:brainstorming"
```
...
Personal skills override opc-superpowers skills with the same name.

## Updating

```bash
cd ~/.config/opencode/opc-superpowers
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/opc-superpowers/.opencode/plugin/opc-superpowers.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/opc-superpowers/skills`
...
- Documentation: https://github.com/kaelzhang/opc-superpowers
```
...
opc-superpowers works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
**Files:**
- Test: `.codex/opc-superpowers-codex`
...
Run: `.codex/opc-superpowers-codex use-skill opc-superpowers:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/opc-superpowers-codex bootstrap | head -30`
...
ls -l .opencode/plugin/opc-superpowers.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/opc-superpowers.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/opc-superpowers-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/opc-superpowers-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/opc-superpowers.js` created with tools and hooks
