# opc-superpowers Release Notes

## v5.0.7 (2026-03-31)

### GitHub Copilot CLI Support

- **SessionStart context injection** — Copilot CLI v1.0.11 added support for `additionalContext` in sessionStart hook output. The session-start hook now detects the `COPILOT_CLI` environment variable and emits the SDK-standard `{ "additionalContext": "..." }` format, giving Copilot CLI users the full opc-superpowers bootstrap at session start. (Original fix by @culinablaz in PR #910)
- **Tool mapping** — added `references/copilot-tools.md` with the full Claude Code to Copilot CLI tool equivalence table
- **Skill and README updates** — added Copilot CLI to the `using-opc-superpowers` skill's platform instructions and README installation section

### OpenCode Fixes

- **Skills path consistency** — the bootstrap text no longer advertises a misleading `configDir/skills/opc-superpowers/` path that didn't match the runtime path. The agent should use the native `skill` tool, not navigate to files by path. Tests now use consistent paths derived from a single source of truth. (#847, #916)
...
- **Added `package.json`** so OpenCode can install opc-superpowers as an npm package from git.
...
- `GEMINI.md` @imports `using-opc-superpowers` skill and tool mapping table at session start
- Gemini CLI tool mapping reference (`skills/using-opc-superpowers/references/gemini-tools.md`) — translates Claude Code tool names (Read, Write, Edit, Bash, etc.) to Gemini CLI equivalents (read_file, write_file, replace, etc.)
...
- Specs (brainstorming output) now save to `docs/opc-superpowers/specs/YYYY-MM-DD-<topic>-design.md`
- Plans (writing-plans output) now save to `docs/opc-superpowers/plans/YYYY-MM-DD-<feature-name>.md`
...
Writing-plans no longer offers a choice between subagent-driven and executing-plans. On harnesses with subagent support (Claude Code, Codex), subagent-driven-development is required. Executing-plans is reserved for harnesses without subagent capability, and now tells the user that opc-superpowers works better on a subagent-capable platform.
...
- `lib/brainstorm-server/` — WebSocket server with browser helper library, session management scripts, and dark/light themed frame template ("opc-superpowers Brainstorming" with GitHub link)
...
- Design spec and implementation plan in `docs/opc-superpowers/`
...
Added explicit priority ordering to using-opc-superpowers:

1. User's explicit instructions (CLAUDE.md, AGENTS.md, direct requests) — highest priority
2. opc-superpowers skills — override default system behavior
...
Added `<SUBAGENT-STOP>` block to using-opc-superpowers. Subagents dispatched for specific tasks now skip the skill instead of activating the 1% rule and invoking full skill workflows.
...
This fix should dramatically improve opc-superpowers skills compliance and should reduce the chances of Claude entering its native plan mode unintentionally.
...
**Using-opc-superpowers workflow graph intercepts EnterPlanMode**
...
Changed `async: true` to `async: false` in hooks.json. When async, the hook could fail to complete before the model's first turn, meaning using-opc-superpowers instructions weren't in context for the first message.
...
The `opc-superpowers-codex` bootstrap CLI, Windows `.cmd` wrapper, and related bootstrap content file have been removed. Codex now uses native skill discovery via `~/.agents/skills/opc-superpowers/` symlink, so the old `use_skill`/`find_skills` CLI tools are no longer needed.
...
The synchronous SessionStart hook blocked the TUI from entering raw mode on Windows, freezing all keyboard input. Running the hook async prevents the freeze while still injecting opc-superpowers context.
...
- Windows doesn't respect shebangs, so directly invoking the extensionless `opc-superpowers-codex` script triggered an "Open with" dialog. All invocations now prefixed with `node`.
...
opc-superpowers for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/opc-superpowers/` (see updated installation docs).
...
**Strengthened using-opc-superpowers skill for explicit skill requests**
...
The underlying skills (`opc-superpowers:brainstorming`, `opc-superpowers:executing-plans`, `opc-superpowers:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-opc-superpowers` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-opc-superpowers`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `opc-superpowers:brainstorming`)
...
**Skill priority in using-opc-superpowers**
...
- **Rationalizations table** in using-opc-superpowers - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-opc-superpowers is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > opc-superpowers
...
- Optimized opc-superpowers bootstrap to eliminate redundant skill execution. The `using-opc-superpowers` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-opc-superpowers` manually despite already having the content from session start.
...
- Added unified `opc-superpowers-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `opc-superpowers:skill-name` for opc-superpowers skills, `skill-name` for personal
- Personal skills override opc-superpowers skills when names match
...
- Updated terminology: "opc-superpowers skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/opc-superpowers-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/opc-superpowers-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core opc-superpowers functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-opc-superpowers skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-opc-superpowers/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-opc-superpowers skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-opc-superpowers/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `opc-superpowers:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `opc-superpowers:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `opc-superpowers:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `opc-superpowers:code-reviewer`
...
- All internal skill references now use `opc-superpowers:` namespace prefix
- Updated format: `opc-superpowers:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/opc-superpowers:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# opc-superpowers v2.0.0 Release Notes

## Overview

opc-superpowers v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([kaelzhang/opc-superpowers-skills](https://github.com/kaelzhang/opc-superpowers-skills)). This transforms opc-superpowers from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [kaelzhang/opc-superpowers-skills](https://github.com/kaelzhang/opc-superpowers-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/opc-superpowers/skills/`
...
1. Your old `~/.config/opc-superpowers/.git` will be backed up to `~/.config/opc-superpowers/.git.bak`
2. Old skills will be backed up to `~/.config/opc-superpowers/skills.bak`
3. Fresh clone of kaelzhang/opc-superpowers-skills will be created at `~/.config/opc-superpowers/skills/`

### Removed Features

- **Personal opc-superpowers overlay system** - Replaced with git branch workflow
- **setup-personal-opc-superpowers hook** - Replaced by initialize-skills.sh
...
- Clones kaelzhang/opc-superpowers-skills on first run
...
- `OPC_SUPERPOWERS_SKILLS_ROOT` set to `~/.config/opc-superpowers/skills`
...
- Removed obsolete setup-personal-opc-superpowers call from session-start
...
- Prominent link to opc-superpowers-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in kaelzhang/opc-superpowers-skills
- `scripts/` directory - Now in kaelzhang/opc-superpowers-skills/skills/using-skills/
- `hooks/setup-personal-opc-superpowers.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/opc-superpowers/skills
- `commands/brainstorm.md` - Updated paths to OPC_SUPERPOWERS_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to OPC_SUPERPOWERS_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to OPC_SUPERPOWERS_SKILLS_ROOT
...
- PR #2: Personal opc-superpowers overlay system (later replaced)
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/opc-superpowers/skills ~/opc-superpowers-skills-backup
   ```

2. **Update the plugin:**
   ```bash
   /plugin update opc-superpowers
   ```
...
- Skills repository is now at https://github.com/kaelzhang/opc-superpowers-skills
...
**Full Changelog:** https://github.com/kaelzhang/opc-superpowers/compare/dd013f6...main
**Skills Repository:** https://github.com/kaelzhang/opc-superpowers-skills
