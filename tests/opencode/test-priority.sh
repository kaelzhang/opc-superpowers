...
# Verifies that skills are resolved with correct priority: project > personal > opc-superpowers
...
# 1. Create in opc-superpowers location (lowest priority)
mkdir -p "$OPC_SUPERPOWERS_SKILLS_DIR/priority-test"
cat > "$OPC_SUPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: opc-superpowers version of priority test skill
---
# Priority Test Skill (opc-superpowers Version)

This is the OPC_SUPERPOWERS version of the priority test skill.

PRIORITY_MARKER_OPC_SUPERPOWERS_VERSION
EOF
...
if [ -f "$OPC_SUPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] opc-superpowers version exists"
else
    echo "  [FAIL] opc-superpowers version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides opc-superpowers
echo ""
echo "Test 2: Testing personal > opc-superpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides opc-superpowers)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_OPC_SUPERPOWERS_VERSION"; then
    echo "  [FAIL] opc-superpowers version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|opc-superpowers" | head -10
fi

# Test 3: Test that project overrides both personal and opc-superpowers
echo ""
echo "Test 3: Testing project > personal > opc-superpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] opc-superpowers version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit opc-superpowers: prefix bypasses priority
echo ""
echo "Test 4: Testing opc-superpowers: prefix forces opc-superpowers version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load opc-superpowers:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_OPC_SUPERPOWERS_VERSION"; then
    echo "  [PASS] opc-superpowers: prefix correctly forces opc-superpowers version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] opc-superpowers: prefix did not force opc-superpowers version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
